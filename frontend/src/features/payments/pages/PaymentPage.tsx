...
export function PaymentPage() {
  ...
  if (organizationRequiredForPayment && !selectedOrganizationId) return <main className={tw('route-message')}><Building2 size={34}/><h1>Create an organization before payment</h1><p>Licenses and licensed radio products are owned by an organization. Your cart will remain saved while you create one.</p><div className="grid justify-items-center gap-5"><Link className={tw('primary-action')} to="/account?tab=licenses">Create organization</Link><Link className={tw('text-link')} to="/cart"><ArrowLeft size={15}/>Back to cart</Link></div></main>
