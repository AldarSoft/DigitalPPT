      ...
      <table>
        ...
        <tbody>
          {loading ? <tr><td colSpan={6}>Loading quote requests...</td></tr> : quotes.length ? quotes.map((quote) => (
            <tr className={tw(`record-row ${['invoice_sent', 'awaiting_payment', 'payment_rejected'].includes(quote.status) && quote.order_status === 'pending' ? 'bg-warning-soft hover:bg-warning-soft' : ''}`)} key={quote.id} onDoubleClick={() => onSelect?.(quote)}>
              ...
              <td><span className={tw('mobile-table-label')}>Status</span><span className={tw(`status status-${quoteStatusKey(quote.status, quote.order_status)}`)}>{quoteStatusLabel(quote.status, quote.order_status)}</span></td>
              <td><span className={tw('mobile-table-label')}>Action</span><button className={tw('table-action')} type="button" onClick={() => onSelect?.(quote)}>{['invoice_sent', 'awaiting_payment', 'payment_rejected'].includes(quote.status) && quote.order_status === 'pending' ? 'Review invoice' : 'View'}</button></td>
            </tr>
