      ...
      <table>
        ...
        <tbody>
          {loading ? <tr><td colSpan={6}>Loading quote requests...</td></tr> : quotes.length ? quotes.map((quote) => (
            <tr className={tw(`record-row ${quote.status === 'quoted' && quote.order_status === 'pending' ? 'bg-warning-soft hover:bg-warning-soft' : ''}`)} key={quote.id} onDoubleClick={() => onSelect?.(quote)}>
              ...
              <td><span className={tw('mobile-table-label')}>Action</span><button className={tw('table-action')} type="button" onClick={() => onSelect?.(quote)}>{quote.status === 'quoted' && quote.order_status === 'pending' ? 'Review invoice' : 'View'}</button></td>
            </tr>
