...
export function AccountRecordDialog({ record, onClose, onLinkedQuoteSelect, paymentsEnabled = false, organizationId = null }: { record: AccountRecord; onClose: () => void; onLinkedQuoteSelect?: (quoteNumber: string) => void; paymentsEnabled?: boolean; organizationId?: number | null }) {
  useEffect(() => {
  ...
  return (
    <div className={tw('modal-backdrop')} role="presentation" onMouseDown={onClose}>
      <section className={tw('account-record-dialog')} role="dialog" aria-modal="true" aria-labelledby="account-record-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {isOrder ? <ClientOrderDetails order={record.value} organizationId={organizationId} paymentsEnabled={paymentsEnabled} onLinkedQuoteSelect={onLinkedQuoteSelect} /> : <ClientQuoteDetails initialQuote={record.value} organizationId={organizationId} paymentsEnabled={paymentsEnabled} />}
      </section>
    </div>
  )
}
...
function ClientQuoteDetails({ initialQuote, paymentsEnabled, organizationId }: { initialQuote: QuoteRequest; paymentsEnabled: boolean; organizationId: number | null }) {
  ...
  return (<>
    ...
    {quote.status === 'quoted' && quote.invoice_pdf_url ? <section className="mt-5 rounded-control border border-warning bg-warning-soft p-4 text-sm text-warning"><h3 className="text-base font-extrabold text-ink">Invoice ready for payment</h3><p className="mt-1">{quote.invoice_number} created order {quote.order_number || 'for this quote'} and is waiting for payment.</p><div className="mt-3 flex flex-wrap gap-2"><a className={`${tw('record-payment-link')} !text-white [&>svg]:text-white`} href={mediaUrl(quote.invoice_pdf_url)} target="_blank" rel="noreferrer"><Download size={17} />Download PDF</a>{paymentsEnabled && quote.order_number ? <Link className={`${tw('record-payment-link')} !text-white [&>svg]:text-white`} to={`/payment?order=${encodeURIComponent(quote.order_number)}${organizationId ? `&org=${organizationId}` : ''}`}><CreditCard size={17} />Pay invoice</Link> : null}</div></section> : null}
