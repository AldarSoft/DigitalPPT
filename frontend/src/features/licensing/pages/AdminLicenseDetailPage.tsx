...
function OrganizationUsersDialog({ organizationId, data, loading, error, onClose, onRetry, onChanged }: { organizationId: number; data: AdminOrganizationUsers | undefined; loading: boolean; error: unknown; onClose: () => void; onRetry: () => void; onChanged: () => void }) {
  ...
  </Dialog>{ownerCandidate ? <Dialog title="Confirm ownership transfer" onClose={transfer.isPending ? () => undefined : () => setOwnerCandidate(null)}><div className="grid gap-4"><div className="flex items-start gap-3 rounded-control border border-danger bg-danger-soft px-3 py-3 text-danger"><AlertTriangle className="mt-0.5 shrink-0" size={20} /><div><strong className="block text-sm">Make {ownerCandidate.name} the Organization Owner?</strong><p className="mt-1 text-sm">The current Owner will become a License Manager. The new Owner will receive full organization, billing, and team control.</p></div></div>{transfer.error ? <p className="rounded-control bg-danger-soft px-3 py-2 text-sm text-danger">{messageFrom(transfer.error)}</p> : null}<div className="flex justify-end gap-3"><button className="min-h-10 rounded-control border border-border-input bg-white px-4 text-sm font-bold" type="button" disabled={transfer.isPending} onClick={() => setOwnerCandidate(null)}>Cancel</button><button className="min-h-10 rounded-control border-0 bg-danger px-4 text-sm font-bold text-white disabled:opacity-55" type="button" disabled={transfer.isPending} onClick={() => transfer.mutate(ownerCandidate.membership_id)}>{transfer.isPending ? 'Transferring...' : 'Confirm transfer'}</button></div></div></Dialog> : null}</>
}
