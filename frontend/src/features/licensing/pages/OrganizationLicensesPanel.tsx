...
import type { ClientLicenseDetail, LicenseStatus, OrganizationLicenseStatus } from '../types'
...
export function OrganizationLicensesPanel({ organizationId, workspaceLoading, workspaceError, onWorkspaceRetry }: { organizationId: number | null; workspaceLoading: boolean; workspaceError: unknown; onWorkspaceRetry: () => void }) {
  ...
  const cancellationMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      closeDetails()
      void queryClient.invalidateQueries({ queryKey: licensingKeys.licenses(organizationId) })
      void queryClient.invalidateQueries({ queryKey: licensingKeys.summary(organizationId) })
      toast.success('License cancelled and recorded in organization history')
    },
  })
  ...
  return (
    <div className="grid gap-4">
      <header className="grid min-w-0 gap-3 min-[680px]:grid-cols-[minmax(0,1fr)_auto] min-[680px]:items-start">
        <div className="min-w-0"><h2 className="text-[28px] leading-tight max-[480px]:text-[25px]">Organization licenses</h2><p className="mt-1 text-sm text-muted">View covered radios, source orders, and renewal dates.</p></div>
      ...
      <div className="flex items-start gap-3 rounded-panel border border-[#bfd5f5] bg-info-soft px-4 py-3 text-sm text-[#244368]"><Info className="mt-0.5 shrink-0 text-brand" size={19} /><p>Each paid radio order creates one license covering the radios in that order. Later purchases create separate licenses with their own renewal dates.</p></div>
      {licensesQuery.data?.summary.overflow_quantity ? <div className="flex items-start gap-3 rounded-panel border border-danger bg-danger-soft px-4 py-3 text-sm text-danger"><AlertTriangle className="mt-0.5 shrink-0" size={19} /><div><strong className="block">Radio coverage required</strong><p className="mt-1">{licensesQuery.data.summary.overflow_quantity} purchased radio product(s) do not have usable compatible license coverage. This warning remains until coverage is restored.</p></div></div> : null}
      ...
      {licensesQuery.isLoading ? <StatePanel icon={<LoaderCircle className="animate-spin text-brand" size={22} />} title="Loading organization licenses" text="Retrieving covered radios, expiry, and subscription information." /> : null}
        ...
        <section className="min-w-0 rounded-panel border border-border bg-white p-5 max-[560px]:p-4">
          ...
          {licenses.map((license) => {
            ...
            return <article aria-current={selected ? 'true' : undefined} className={`grid min-w-0 gap-4 border-b border-border px-3 py-5 last:border-b-0 min-[460px]:grid-cols-2 min-[680px]:grid-cols-[minmax(0,1.2fr)_minmax(120px,.8fr)_minmax(112px,.6fr)_92px] min-[680px]:items-center min-[680px]:gap-3 ${selected ? 'rounded-control bg-info-soft ring-1 ring-[#bdd4f5]' : ''}`} key={license.id}>
              ...
              <div className="min-w-0"><span className="mb-1 block text-xs font-bold text-muted min-[680px]:hidden">{license.billing_model === 'per_radio_order' ? 'Radios covered' : 'Product capacity'}</span>{license.billing_model === 'per_radio_order' ? <strong>{license.covered_radio_count} radio{license.covered_radio_count === 1 ? '' : 's'}</strong> : <><strong>{license.used_capacity} / {license.capacity}</strong><div className="mt-2 h-2 overflow-hidden rounded-full bg-border" role="progressbar" aria-label={`${license.name} capacity`} aria-valuemin={0} aria-valuemax={license.capacity} aria-valuenow={license.used_capacity}><span className="block h-full rounded-full bg-brand" style={{ width: `${license.capacity_percentage}%` }} /></div></>}</div>
    ...
    <section aria-labelledby="license-details-title" aria-modal="true" className="max-h-[calc(100vh-32px)] w-full max-w-4xl overflow-y-auto rounded-panel border border-border bg-white shadow-xl" role="dialog" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {detail ? <div className="grid gap-5 p-5 max-[560px]:p-4">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(260px,.72fr)]">
          {detail.billing_model === 'per_radio_order' ? <div className="rounded-panel bg-surface-raised p-4"><span className="text-sm font-bold text-muted">Radios covered by this order</span><div className="mt-2 flex items-end gap-2"><b className="text-3xl text-ink">{detail.covered_radio_count}</b><span className="pb-1 text-sm font-semibold text-muted">radio{detail.covered_radio_count === 1 ? '' : 's'}</span></div><p className="mt-3 text-sm text-muted">This quantity is fixed by the completed source order and renews together.</p></div> : <div className="rounded-panel bg-surface-raised p-4"><div className="flex items-end justify-between gap-3"><strong>Products assigned</strong><b className="text-2xl">{detail.used_capacity} / {detail.capacity}</b></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-border"><span className="block h-full rounded-full bg-success" style={{ width: `${Math.min(100, detail.capacity ? detail.used_capacity / detail.capacity * 100 : 0)}%` }} /></div><p className="mt-3 text-sm font-semibold text-success">{detail.available_capacity === 0 ? 'This license is at full capacity.' : `${detail.available_capacity} product capacity available.`}</p></div>}
...
function LicenseCancellationDialog({ license, pending, error, onClose, onConfirm }: { license: ClientLicenseDetail; pending: boolean; error: unknown; onClose: () => void; onConfirm: (password: string, reason: string) => void }) {
  ...
  return <div className="fixed inset-0 z-[60] grid place-items-center bg-[#09172d]/45 p-4" role="presentation" onMouseDown={() => { if (!pending) onClose() }}>
    <section aria-labelledby="cancel-license-title" aria-modal="true" className="w-full max-w-lg rounded-panel border border-border bg-white p-5 shadow-xl" role="dialog" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="mt-4 flex items-start gap-3 rounded-control border border-danger bg-danger-soft p-3 text-sm text-danger"><AlertTriangle className="mt-0.5 shrink-0" size={19} /><p>{license.billing_model === 'per_radio_order' ? `This removes coverage for ${license.covered_radio_count} radio${license.covered_radio_count === 1 ? '' : 's'} immediately.` : `This removes ${license.capacity} usable capacity immediately.`} Purchase and allocation history remains in the audit log, and uncovered radios will require a replacement license.</p></div>
      <form className="mt-4 grid gap-4" onSubmit={(event) => { event.preventDefault(); onConfirm(password, reason) }}>
        ...
        <label className="flex items-start gap-2.5 text-sm text-muted"><input className="mt-0.5 size-4 shrink-0 accent-danger" type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span>I understand the radio coverage is removed immediately and this action cannot be undone.</span></label>
...
function StatusFeedback({ status, remainingDays }: { status: LicenseStatus; remainingDays: number | null }) {
  const feedbackByStatus: Record<OrganizationLicenseStatus, { tone: string; text: string }> = {
    ...
    no_licenses: { tone: 'bg-surface-muted text-muted', text: 'This organization has no licenses.' },
  }
