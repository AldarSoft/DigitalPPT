...
import { AlertTriangle, CalendarDays, CreditCard, Info, KeyRound, LoaderCircle, RefreshCw, ShieldCheck, X } from 'lucide-react'
...
export function OrganizationLicensesPanel({ organizationId }: { organizationId: number | null }) {
  ...
  return (
    <div className="grid gap-4">
      <header className="flex flex-wrap items-start justify-between gap-4 max-[560px]:gap-3">
        ...
        <span className="inline-flex items-center gap-2 rounded-control bg-surface-muted px-3 py-2 text-xs font-bold text-muted max-[560px]:w-full"><KeyRound size={17} className="shrink-0 text-brand" />Owner &amp; License Manager access</span>
      </header>
      ...
      <div className="grid items-start gap-4">
        <section className="min-w-0 rounded-panel border border-border bg-white p-5 max-[560px]:p-4">
          ...
          {licenses.map((license) => {
            ...
            return <article aria-current={selected ? 'true' : undefined} className={`grid gap-4 border-b border-border px-3 py-5 last:border-b-0 min-[460px]:grid-cols-2 md:grid-cols-[minmax(0,1.2fr)_minmax(130px,.8fr)_minmax(120px,.6fr)_84px] md:items-center ${selected ? 'rounded-control bg-info-soft ring-1 ring-[#bdd4f5]' : ''}`} key={license.id}>
              <div className="min-w-0 min-[460px]:col-span-2 md:col-span-1"><strong className="block truncate">{license.name}</strong><span className="text-xs text-muted">{license.license_number}</span></div>
              <div><span className="mb-1 block text-xs font-bold text-muted md:hidden">Product capacity</span><strong>{license.used_capacity} / {license.capacity}</strong><div className="mt-2 h-2 overflow-hidden rounded-full bg-border" role="progressbar" aria-label={`${license.name} capacity`} aria-valuemin={0} aria-valuemax={license.capacity} aria-valuenow={license.used_capacity}><span className="block h-full rounded-full bg-brand" style={{ width: `${license.capacity_percentage}%` }} /></div></div>
              <div className="grid justify-items-start gap-2"><span className="block text-xs font-bold text-muted md:hidden">Expires</span><strong className="block text-sm">{formatDate(license.expires_on)}</strong><LicenseStatusBadge status={license.status} /></div>
              <button className={`${selected ? '!bg-brand !text-white' : '!bg-brand-soft !text-brand'} inline-flex min-h-10 items-center justify-center rounded-control px-3 text-xs font-bold max-[459px]:w-full`} type="button" onClick={() => selectLicense(license.license_number)}>View details</button>
            </article>
          })}
        </section>
      </div>
...
function RetryButton({ onClick }: { onClick: () => void }) {
  return <button className="inline-flex min-h-9 items-center gap-2 rounded-control border border-border-input bg-white px-3 text-xs font-bold text-brand" type="button" onClick={onClick}><RefreshCw size={15} />Try again</button>
}
