...
export function OrderRows({ orders, compact = false, onSelect }: {
    ...
    return (<div className={tw("admin-table-wrap")}>
      <table className={tw(`admin-table ${compact ? 'admin-table-compact' : ''}`)}>
        ...
        <tbody>{orders.length ? orders.map((order) => (<tr className={tw('record-row')} key={order.id} onDoubleClick={() => onSelect?.(order)}><td><button className={tw("view-order")} type="button" onClick={() => onSelect?.(order)}>{order.order_number}</button></td><td>{orderSourceLabel(order.source)}</td><td>{order.customer_first_name} {order.customer_last_name}</td><td>{new Date(order.created_at).toLocaleDateString()}</td><td>${Number(order.total).toFixed(2)}</td><td><span className={tw(`status status-${orderStatusKey(order.status)}`)}>{orderStatusLabel(order.status)}</span></td>{compact ? null : <td><button className={tw("view-order")} type="button" onClick={() => onSelect?.(order)}>View</button></td>}</tr>)) : <tr><td colSpan={compact ? 6 : 7}>No orders found.</td></tr>}</tbody>
      </table>
    </div>);
}
