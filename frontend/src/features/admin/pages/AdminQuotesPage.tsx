      ...
      {selected ? (
        <div className={tw('editor-backdrop')} role="presentation" onMouseDown={closeQuote}>
          <aside className={tw('order-editor')} role="dialog" aria-modal="true" aria-labelledby="quote-details-title" onMouseDown={(event) => event.stopPropagation()}>
            ...
            {selected.coverage_targets?.length ? <section className="mt-5 border-t border-border-soft pt-5"><h3 className="mb-3 text-sm font-extrabold">Requested radio coverage</h3><div className="grid gap-2">{selected.coverage_targets.map((target) => <div className="flex items-center justify-between gap-4 rounded-control bg-surface-raised px-3 py-2 text-sm" key={target.order_item_id}><span className="min-w-0"><strong className="block truncate">{target.product_name}</strong><small className="text-text-soft">{target.product_sku || 'Radio'} · {target.order_number}</small></span><strong className="shrink-0">Qty {target.quantity}</strong></div>)}</div></section> : null}
